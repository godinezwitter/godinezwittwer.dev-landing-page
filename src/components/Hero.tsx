    ...
    >
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-6 py-20 md:py-28">
        <div className="grid grid-cols-1 lg:grid-cols-[1.15fr_0.85fr] gap-14 lg:gap-16 items-center">
          ...
          {/* Right — "how we work", as a paper surface card echoing the About /
              contact panels. Desktop only: on narrow screens there's no column
              for it and the pitch carries the hero on its own. */}
          <motion.div
          ...
          >
            <div className="surface rounded-3xl p-8">
              <p className="kicker mb-6">{t.hero.panelTitle}</p>
              <ol className="flex flex-col">
                {t.hero.panel.map(({ title, detail }, i) => (
                  <li
                    key={title}
                    className="grid grid-cols-[auto_1fr] gap-x-4 py-4"
                    style={i > 0 ? { borderTop: "1px solid var(--color-line-ink)" } : undefined}
                  >
                    <span
                      className="font-body font-semibold tabular-nums leading-none pt-0.5"
                      style={{ fontSize: "1.05rem", color: "var(--color-wine)", letterSpacing: "-0.02em" }}
                      aria-hidden="true"
                    >
                      0{i + 1}
                    </span>
